...
class kMatrix (kArrayCommon):
    ...
    def __iter__(self):
        for r in range(self.array.shape[0]):
            for c in range(self.array.shape[1]):
                yield self.array[r,c]

    @property
    def T(self):
        return kMatrix( self.array.T )

    #( --- sum --- )#
    def __add__(self, y):
        if isinstance(y, kMatrix):
            ret = kMatrix( self.array + y.array )
        elif isinstance(y, float) or isinstance(y, int):
            ret = kMatrix( self.array + y )
        else:
            raise(NameError("not prepared for type '{:s}'".format(str(type(y)))))
        return ret
    ...
    #( --- multiplication --- )#
    def __mul__(self, y):
        if isinstance(y, kMatrix): # M x M
            ret = kMatrix( np.dot(self.array, y.array) )
        elif isinstance(y, kVector): # M x V
            ret = kVector( np.dot(self.array, y.array), hvector=False )
        elif isinstance(y, int) or isinstance(y, float): # k x M
            ret = kMatrix( y * self.array )
        else:
            raise(NameError("not prepared for type '{:s}'".format(str(type(y)))))

        return ret

    def __rmul__(self, y): # y x M
        if isinstance(y, kMatrix): # M x M
            ret = kMatrix( np.dot(y.array, self.array) )
        elif isinstance(y, kVector): # V x M
            ret = kVector( np.dot(y.array, self.array), hvector=True )
        elif isinstance(y, int) or isinstance(y, float): # k x M
            ret = kMatrix( y * self.array )
        else:
            raise(NameError("not prepared for type '{:s}'".format(str(type(y)))))
